...
function rectangle()
{
    ...
    for (i = 0; i <= xRes; i++) {
        for (j = 0; j <= yRes; j++) {
            ...
            // X
            x = (R + psi * Math.cos(phi / 2) * r) * Math.cos(phi);
            // Y
            y = (R + psi * Math.cos(phi / 2) * r) * Math.sin(phi);
            // Z
            z = Math.cos(psi) * (Math.sin(phi) / 2) * r;
            points.push(vec3(x, y, z));
            UVs.push(phi / Math.PI * 5., psi / Math.PI);
            normals.push(Math.cos(phi) * Math.cos(psi), Math.sin(phi) * Math.cos(phi), Math.sin(phi));
        }
    }
...
function render()
{
    ...
    gl.uniform1i(gl.getUniformLocation(program, "Specular"), true);
...
function OnKeyDown(event)
{
    ...
    if (event.keyCode == 80) {
        if(pauseKey == true)
        {
            velocity_y = velocityMove;
            pauseKey = false;
        }
        else{
            velocity_y = velocityStop;
            pauseKey = true;
        }
    }
}
